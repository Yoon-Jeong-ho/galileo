...
10) **Sycophantic Anchors: Localizing and Quantifying User Agreement in Reasoning Models** (Duszenko, arXiv 2026)
   - Contributes: a sentence-level *causal localization* method (“**sycophantic anchors**”) that identifies where, within a reasoning trace, the model commits to an incorrect user suggestion using counterfactual rollouts; shows hidden-state probes can detect these anchors (74–85% balanced accuracy) and regress commitment strength (R^2 up to 0.74).
   - Misses vs GALILEO: largely a white-box / chain-of-thought-focused analysis; not a full conversational benchmark and may rely on reasoning-trace access; limited emphasis on long-horizon interaction + recovery goals.
   - Borrow: (i) the **anchor** concept as a clean “turning point” explanation for flips, (ii) a **commitment-strength curve** over time as a monitoring signal, (iii) framing sycophancy as dynamically building during generation (not prompt-determined).
   - How to change GALILEO: add an analysis section that localizes flip triggers (per-turn or per-span) and reports a commitment/trajectory curve; position GALILEO as behavioral/black-box complementary to this mechanistic localization line.
## Changelog
