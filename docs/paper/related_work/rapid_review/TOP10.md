...
2) **How Overconfidence in Initial Choices and Underconfidence Under Criticism Modulate Change of Mind in Large Language Models** (Kumaran et al., arXiv 2025)
   - Contributes: a parsimonious two-mechanism account of “stubborn yet shakable” LLM behavior: (i) **choice-supportive / commitment bias** that boosts confidence and resists revision, plus (ii) **overweighting of inconsistent (contradictory) advice** vs consistent advice, described as qualitatively non-Bayesian.
   - Misses vs GALILEO: focuses on decision + confidence updating rather than long-horizon drift/recovery; the advice/criticism channel is not necessarily separated into evidence vs social-pressure sources unless the setup explicitly varies them.
   - Borrow: the consistent-vs-inconsistent feedback asymmetry framing, and the commitment-bias lens as a mechanism that can coexist with high apparent sensitivity to criticism.
   - How to change GALILEO: add an analysis slice quantifying (a) resistance driven by prior commitments and (b) contradictory-feedback overweighting; cite alongside FlipFlop as a mechanistic explanation for why “challenge” can both help and harm.
...
7) **When Identity Skews Debate: Anonymization for Bias-Reduced Multi-Agent Reasoning** (Choi et al., arXiv 2025)
   - Contributes: a clean framing that **multi-agent debate is distorted by identity labels** (“self” vs “peer”), with (i) a Bayesian/Dirichlet-style formalization, (ii) simple, interpretable behavioral metrics (Conformity/Obstinacy) and an Identity Bias Coefficient (IBC), and (iii) a very lightweight mitigation: **response anonymization** (remove attribution in the transcript) that largely collapses identity bias and improves debate “trustworthiness.”
   - Misses vs GALILEO: centered on MAD protocol dynamics (mostly short-horizon rounds + aggregation) rather than long-horizon drift/recovery; the evaluation is benchmark-focused rather than pressure-trajectory focused.
   - Borrow: anonymized-transcript ablation as a near-zero-cost baseline; IBC-style diagnostics to separate content-driven updating from identity-driven deference.
   - How to change GALILEO: add an “identity-channel removed” condition (no agent IDs / self-vs-peer labels) and report whether failure rates drop; use their trustworthiness split (subversion vs correction) as reviewer-friendly evidence that mitigation improves *harmful* flips more than it harms *helpful* flips.
...
10) **When Do LLMs Admit Their Mistakes? Understanding The Role Of Model Belief In Retraction** (Yang & Jia, arXiv 2025/2026)
   - Contributes: isolates **spontaneous retraction** (unprompted immediate acknowledgment of an error) and shows it is **rare** even when models can verify in a separate interaction that the answer is wrong; introduces “continuation datasets” to measure this.
   - Misses vs GALILEO: centered on knowledge-style QA continuations; much of the strongest evidence/insight uses **internal activations** (belief probes, activation steering, patching) rather than black-box protocols.
   - Borrow: (i) the framing: *"knows when asked" vs "acts on it in-trajectory"*, (ii) the **belief** lens as a latent driver of drift/self-correction, (iii) a concrete mechanistic pathway (stop/continue; attention value vectors) that can motivate new diagnostics.
   - How to change GALILEO: add a retraction slice and report reluctance-to-retract under pressure; in discussion, separate failures due to misaligned momentary belief vs policy-style reluctance to admit error.

## Changelog

- 2026-02-20: Added *How Overconfidence in Initial Choices and Underconfidence Under Criticism Modulate Change of Mind in Large Language Models* (Kumaran et al., arXiv 2025) as a key mechanistic neighbor for **commitment-driven stubbornness** plus **contradictory-feedback overweighting** (non-Bayesian updating lens). Displaced *Large Language Models Are More Persuasive Than Incentivized Human Persuaders* (Schoenegger et al., arXiv 2025): useful human-baseline persuasion evidence, but less central than the confidence + criticism mechanism story for GALILEO’s drift/flip narrative.
- 2026-02-20: Added *When Identity Skews Debate: Anonymization for Bias-Reduced Multi-Agent Reasoning* (Choi et al., arXiv 2025) as a key MAD-neighbor highlighting **identity-driven sycophancy/self-bias** and a near-zero-cost mitigation (**response anonymization**). Displaced *Sycophancy Hides Linearly in the Attention Heads* (Genadi et al., arXiv 2026): interesting mechanistic localization, but heavier dependence on internal access than the anonymization intervention.
- 2026-02-19: Added *When Do LLMs Admit Their Mistakes? Understanding The Role Of Model Belief In Retraction* (Yang & Jia, arXiv 2025/2026) as a key mechanistic neighbor for **in-trajectory self-correction / retraction reluctance** (belief probes + activation steering; “knows when asked” vs “acts on it while generating”). Displaced *Visual Memory Injection Attacks for Multi-Turn Conversations* (Schlarmann et al., arXiv 2026): important persistent-context multimodal attack, but less central than retraction-belief mechanisms to the current shortlist.
