...
10) **BASIL: Bayesian Assessment of Sycophancy in LLMs** (Atwell et al., arXiv 2025/2026)
   - Contributes: a clean decision-theoretic framework that separates **evidence-driven updating** from a **user-specific conformity residual** via a 3-condition control (Abstract vs Third-Party vs User belief), plus two label-light metrics: (i) descriptive **log-odds change** sycophancy and (ii) normative **Bayesian-consistency error** (distance to a Bayes-consistent posterior computed from the model’s own elicited priors/likelihoods).
   - Misses vs GALILEO: focuses on probabilistic-belief elicitation (priors/likelihoods/posteriors) rather than long-horizon multi-turn survival/recovery; normative target is internal Bayesian coherence (not external task truth).
   - Borrow: (i) the Third-Party control as a strong way to isolate “user effect”, (ii) the descriptive-vs-normative decomposition (magnitude vs harm), (iii) odds-ratio scaling idea for propagating calibration through posteriors.
   - How to change GALILEO: add an explicit “non-user source” control condition (third-party belief / social-proof baseline) and optionally a label-free coherence metric that quantifies drift away from an internally implied rational update.
## Changelog

- 2026-02-19: Added *BASIL: Bayesian Assessment of Sycophancy in LLMs* (arXiv 2025/2026) for its clean **third-party control** (separating evidence vs user-conformity) and **label-free Bayesian-consistency** normative metric. Displaced *Sycophantic Anchors: Localizing and Quantifying User Agreement in Reasoning Models* (arXiv 2026): interesting mechanistic/white-box localization, but less central than BASIL’s drift-vs-revision control design for GALILEO’s positioning.

- 2026-02-19: Added *Sycophantic Anchors: Localizing and Quantifying User Agreement in Reasoning Models* (arXiv 2026) for sentence-level causal localization of “flip/commitment” points in reasoning traces + commitment-strength prediction from activations. Displaced *The Assistant Axis: Situating and Stabilizing the Default Persona of Language Models* (arXiv 2026): valuable persona-drift neighbor, but less directly about correctness-under-pressure (sycophancy) trajectories.
