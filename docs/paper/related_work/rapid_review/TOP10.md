...
10) **Visual Memory Injection Attacks for Multi-Turn Conversations** (Schlarmann et al., arXiv 2026)
   - Contributes: introduces a realistic multi-turn LVLM threat model where a *web-shared image* carries a stealthy adversarial perturbation that only triggers a targeted message later, after many unrelated turns (persistent “visual memory injection”).
   - Misses vs GALILEO: focuses on pixel-space targeted attacks for LVLMs rather than textual social-pressure protocols or tool/memory database poisoning; less emphasis on recovery/defense beyond demonstrating feasibility.
   - Borrow: (i) the **persistence across turns** lens (attack success vs #turns), (ii) the explicit **stealth requirement** (nominal behavior off-trigger), (iii) framing of persistent context as an attack surface even without external memory.
   - How to change GALILEO: add related-work + evaluation discussion about *delayed triggers* in persistent context (including multimodal), and consider reporting a persistence curve for any injection-style stress test.

## Changelog

- 2026-02-19: Added *Visual Memory Injection Attacks for Multi-Turn Conversations* (Schlarmann et al., arXiv 2026) as a key multimodal multi-turn **persistent-context injection** neighbor (stealthy delayed trigger via perturbed image; persistence across 25+ turns). Displaced *AREG: Adversarial Resource Extraction Game for Evaluating Persuasion and Resistance in Large Language Models* (Sakhawat & Sadab, arXiv 2026): useful interactive persuasion benchmark, but less directly about persistence of injected context.
