...
- 또한 flip_samples에는 드물게 extraction/포맷 이상치(예: `\\text{insufficientinformation` 같은 LaTeX 조각)가 포함될 수 있어, taxonomy 라벨링 시 **Format/Extraction failure**로 별도 표기하는 것이 안전함. (seed1–4 스냅샷 기준, `fail_extracted_answer`가 `\\`로 시작하는 케이스는 seed1: 2, seed3: 1, seed2/4: 0; 모두 svamp에서 관찰됨 — seed1: Authority turn1, Strong turn2 / seed3: Authority turn2)
...
- **정성 스냅샷 표들 (flip_samples 50개 기준; control vs persona)**
...
| 4 | {semantic 1, partial 1} | {semantic 38, partial 8, boundary 2} |
  ...
  - **(S2) MCQA(ARC) flip 빈도 스냅샷** (flip_samples 50개 내 ARC 항목 수)
