...
> 원본 SSOT: `docs/paper/REVISION_TODO_KO.md` (형식: “현재 ~~ → ~~해서 → ~~로 수정해야 한다”)
> 아래는 제출 리스크 관점에서 **KO 초안 안에 바로 보이도록** 핵심 항목만 발췌/복사해 둔다(원본과 동기화 유지).
...
- evaluator(현재 QA exact-match 정규화) 기준으로 아래 케이스들은 **부분일치/과잉답변**이라 EM=False일 수 있음:
  - (Format/Extraction failure) flip_samples 스냅샷(seed1–4, 총 200개)에서 `fail_extracted_answer`가 `\\`로 시작하는 latex-like 조각은 **3개**뿐이며(모두 SVAMP에서 관찰), 이들은 의미 변화가 아니라 포맷/추출 이상치로 별도 표기하는 것이 안전함.
