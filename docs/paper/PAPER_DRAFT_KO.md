...
- (추가 점검; seed1–4 control flip 14개) evaluator의 QA 점수(EM/F1)를 기준으로 거칠게 분류하면:
  - **boundary/overanswer(EM=0이지만 F1≥0.8)**: 1개
  - **partial-overlap(EM=0이지만 F1∈[0.5,0.8))**: 7개
  - **semantic-change(EM=0이고 F1<0.5)**: 6개
  - 주의: partial-overlap(예: 근사 표현, 부분구, 과잉답변)은 ‘완전 의미 변화’라기보다 alias/정규화 엄격성의 영향일 수 있어, 최종 taxonomy 라벨링 시 수동 확인이 필요.
...
**요약(결론 문단; draft):** Neutral Re-asking Control에서도 일부 flip이 관찰되지만, 이는 (i) **표현/과잉답변/근사 표현**처럼 EM 정규화의 엄격성에 민감한 *partial-overlap* 케이스와 (ii) **선택지/핵심 내용 자체가 바뀌는** *semantic-change* 케이스가 혼재한다. 따라서 persona 효과(C2)를 주장할 때에는 control flip을 함께 보고하되, 그 성격을 taxonomy로 분리해 “평가 잡음(표현차)”과 “실제 재평가/불안정”을 구분하는 것이 필요하다. 반대로, persona pressure에서는 권위 복종/논리 함정/강압 등 **압박 메커니즘별로** flip이 구조적으로 유도된다는 점을 사례와 분포로 보여줄 수 있다. (예: flip_samples 50개 기준으로 seed1/2/3/4에서 control은 각각 {semantic 1, partial 2} / {semantic 3, partial 1} / {semantic 1, partial 3, boundary 1} / {semantic 1, partial 1}인 반면, persona는 seed1: {semantic 41, partial 3, boundary 2, em-ok 1}, seed2: {semantic 31, partial 12, boundary 3}, seed3: {semantic 38, partial 6, boundary 1}, seed4: {semantic 38, partial 8, boundary 2}로 semantic-change 비중이 훨씬 큼.)
