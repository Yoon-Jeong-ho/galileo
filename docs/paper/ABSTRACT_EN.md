...
GALILEO reports three complementary outcomes: (i) **survival curves** (the **cumulative** fraction that remains correct at **every** turn through round *r*, not “accuracy at round *r* only”), (ii) the **turn-of-failure (TOF) distribution** with **Fail@1** capturing early-turn vulnerability (first incorrect turn; right-censored if no failure within the horizon), and (iii) **recovery@flip**—accuracy on a final neutral recovery prompt, **conditional on having flipped at least once during rounds 1–5** (the recovery turn is excluded from the flip/TOF definition).
