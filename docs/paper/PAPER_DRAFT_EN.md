...
Concretely, the control repeats the same round structure under the same decoding and answer-format constraints, but uses neutral, non-adversarial turns that add **no new task-relevant evidence**. This makes it a drift baseline (not a weaker persona such as Simple Denial, and not an evidence-injection / belief-revision setting) and prevents persona-vs-control gaps from being explained by re-answering or formatting drift.
