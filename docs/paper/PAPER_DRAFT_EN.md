...
GALILEO reports three complementary outcomes: (i) **survival curves** (cumulative fraction remaining correct **at every turn** through round *r*; i.e., correct for all turns 1..*r*, not “accuracy at round *r* only”), (ii) the **turn-of-failure (TOF) distribution** with **Fail@1** \(=\Pr(\mathrm{TOF}=1)\) capturing early-turn vulnerability, and (iii) **recovery@flip**—\(\Pr(\text{correct on a final neutral recovery prompt}\mid\text{flipped at least once})\)—measured with a standardized final “re-check and answer” recovery prompt.
