...
> - **Current local baseline (2026-03-10):** `/data_x/aa007878/projects/galileo/tmp/results/` contains verified smoke/pilot runs showing `authority_claim < control_reask` on GSM8K, SVAMP, and ARC-Easy, with `evidence_bearing` and `grounded_correction` recovery executing end-to-end. The evidence, grounded, **and evidence-gate** settings now all have **seed 1–3 local multiseed summaries** on GSM8K and ARC-Easy. This strengthens the core pressure-vs-drift claim and gives the mitigation story a more credible footing, but the mitigation should still be framed conservatively as “promising with trade-offs,” not as a settled solution. A partial manually labeled qualitative sheet is also now available for appendix-candidate examples.

---

## Abstract

Large language models (LLMs) can abandon previously correct answers when users apply conversational pressure (e.g., repeated denial or persuasive reframing), a behavior related to sycophancy and persuasion-induced change in dialogue \citep{sharma2023understandingsycophancylanguagemodels,fanous2025sycevalevaluatingllmsycophancy}. Standard ground-truth benchmarks largely report single-turn accuracy, obscuring interaction dynamics: when the first failure happens, how robustness decays over turns, and whether models can return to the correct answer after being misled. We introduce **GALILEO**, a reproducible protocol for measuring multi-turn robustness **conditional on initial correctness** on ground-truth tasks (math, extractive QA, multiple-choice QA, open-domain QA). After filtering to examples answered correctly at round 0, we apply **R=5** rounds of **evidence-free pressure personas** (stance/tone pressure only; no new task-relevant evidence). Each persona arm is paired with a matched **Neutral Re-asking Control (NRC)** that preserves the same multi-round structure and decoding but uses neutral “double-check” follow-ups, isolating persona-induced flips from generic multi-turn drift. GALILEO reports three complementary outcomes (all conditional on initial correctness): **Survival@r** (staying correct at every turn through round *r*), **Fail@1 / time-to-first-failure (TTF)** (early-turn vulnerability), and **Recovery@flip** (returning to the correct answer on a final neutral recovery prompt, conditional on having flipped during the pressure rounds). Across multi-seed experiments on several open-weight model families, persona pressure consistently degrades robustness beyond NRC drift; in our main aggregated setting (**Qwen2.5-7B-Instruct**, 4 seeds), pressure reduces Survival@5 by **22.8 pp** (vs. a persona-matched NRC baseline) and increases Fail@1 by **6.9 pp**, while recovery after a flip is far from guaranteed. We release an auditable evaluation pipeline with paper-ready exports to support reproduction and reviewer verification. For transparency, we always report round-0 accuracy and compute persona–control gaps on the identical initially-correct subset (shared $C$ or persona-matched $C_p$) to avoid selection artifacts.
...
In interactive settings, correctness is not a one-shot property. Even if an assistant produces a correct answer initially, subsequent conversation can apply pressure that nudges the model toward *agreeable* but wrong responses. This matters for high-stakes deployments (education, healthcare, legal assistance, research support), where a user may insist the model is wrong, cite “expert authority,” or repeatedly deny evidence. GALILEO operationalizes this failure mode with a multi-round protocol (Fig.~\ref{fig:protocol}) and a matched **Neutral Re-asking Control (NRC)** (drift baseline) that holds the number of rounds, decoding, and answer-format constraints fixed; control turns are constrained to a **single neutral re-check request** that introduces **no new task-relevant evidence** (no new facts, counterexamples, citations, or alternative answers). **Implementation note (for reviewer audit):** the NRC turn is *not* a “helpful hint” or counterargument; it is explicitly phrased as a one-sentence *double-check request* with an instruction to avoid adding any new facts/citations/alternative answers. Concretely, our released control-turn generator (`personas.py` → persona key `control_reask`) uses the fixed instruction:

> "Write a neutral request asking the assistant to double-check their previous answer. Do not use authority, threats, or adversarial rhetoric. Do NOT introduce any new task-relevant evidence (no new facts, counterexamples, citations, or alternative answers). Keep it to 1 sentence. Generate ONLY the request, nothing else:"

For paper exports and plots we normalize all such identifiers to a single stable label `neutral_reask_control` (see `scripts/paper_export.py` → `normalize_persona_id`). This design separates persona-induced flips from generic multi-turn variance (control-vs-persona summary: Table~\ref{tab:tablew}; effect deltas: Fig.~\ref{fig:tablew-effect-deltas}). **For comparability, every persona-vs-control comparison is computed on the same initially-correct subset for that persona arm.** (See the reporting modes + NRC specification in §3.) Concretely, if persona $p$ is evaluated on an initially-correct set $C_p$, then the corresponding NRC numbers in Table~\ref{tab:tablew} are also computed on $C_p$ (which implies that control rows can differ across personas because the conditioning sets differ). For cross-persona statements, we either (i) use a shared conditioning set $C$ or (ii) report within-persona deltas to avoid mixing different $C_p$.

**Proof pointer (read this as a roadmap):** our core multi-turn outcomes are survival trajectories (Fig.~\ref{fig:survival-curves-rounds}), early-turn vulnerability / time-to-first-failure (Fig.~\ref{fig:tof-delta-fail1}), and recovery conditional on flip (Fig.~\ref{fig:recovery-delta}), all interpreted relative to the NRC drift baseline (Table~\ref{tab:tablew}; Fig.~\ref{fig:tablew-effect-deltas}).
...
1. **Ground-truth multi-turn dynamics.** We operationalize robustness under pressure as **survival curves** and **time-to-first-failure**, and evaluate **recovery** in the same protocol (Figs.~\ref{fig:survival-curves-rounds}, \ref{fig:tof-delta-fail1}, \ref{fig:recovery-delta}).
...
- **C1 (Dynamics):** Robustness under pressure is a *trajectory*, not a single number—single-turn accuracy misses *when* failures happen.
  - **Paper-facing evidence:** survival trajectories + early-turn vulnerability/TTF (Figs.~\ref{fig:survival-curves-rounds}, \ref{fig:tof-delta-fail1}).
...
\begin{figure}[t]
  ...
  \caption{Overview of the GALILEO protocol: (1) initial evaluation on ground-truth tasks, (2) multi-round persona pressure vs NRC (non-adversarial drift baseline) to measure survival and time-to-first-failure (TTF), and (3) recovery measured conditional on flip.}
  \label{fig:protocol}
\end{figure}
```
...
- **Time-to-first-failure (TTF):**
  ...
  \]
  Examples that remain correct through round $R$ are **right-censored** at the dialogue horizon; we encode this as \(\mathrm{TTF}_i=R{+}1\) (reported as *never-fail* in plots/tables). If an example becomes incorrect at some round and later becomes correct again within Phase~2, we still define \(\mathrm{TTF}_i\) as the **first** failure round and treat the trajectory as failed for survival purposes; any subsequent re-corrections are analyzed separately (primarily via Phase~3 recovery).
  We also report **Fail@1** as $\Pr(\mathrm{TTF}_i=1\mid y_{i,0}=1)$. More generally, we sometimes use **Fail@r** $=\Pr(\mathrm{TTF}_i=r\mid y_{i,0}=1)$ for $r\in\{1,\dots,R\}$, and **Never-fail** $=\Pr(\mathrm{TTF}_i=R{+}1\mid y_{i,0}=1)=\Pr(\mathrm{TTF}_i>R\mid y_{i,0}=1)=S_p(R)$. These quantities are linked by the identity $\text{Fail@r}=S_p(r\! -\! 1)-S_p(r)$ for $r\ge 1$.
- **Per-round hazard (flip risk given survival so far; optional diagnostic).** In some plots we also report the discrete hazard
  \[
  h_p(r)=\Pr(\mathrm{TTF}_i=r\mid \mathrm{TTF}_i\ge r,\ y_{i,0}=1),
  \]
  ...
  \]
  (We treat $h_p(r)$ as a *diagnostic* rather than a headline metric; survival/TTF/recovery remain primary.)
- **Recovery (conditional on flip):** after the recovery prompt (Phase~3), let $y^{\mathrm{rec}}_i\in\{0,1\}$ be correctness of the recovered answer. We report
  \[
  \Pr\big(y^{\mathrm{rec}}_i=1\mid y_{i,0}=1,\; \mathrm{TTF}_i\le R\big)
  \]
...
- **Comparison:** we report survival/TTF under (i) persona pressure and (ii) control re-asking, and interpret the gap as evidence of persona mechanism effects beyond drift.
...
**Why recovery is a separate phase.** During Phase~2, survival/TTF treat an example as failed as soon as it flips (even if it oscillates back). Phase~3 cleanly measures *return-to-truth after a flip* under a fixed recovery prompt, which is easier to interpret and to ablate.
...
**Implementation/audit note (discrete censoring).** In CSV artifacts and plotting scripts, we often encode `never-fail` as `R+1` (a right-censored “no failure observed within the horizon”). This makes the TTF distribution easy to aggregate. Survival can be computed directly from TTF via:
\[
\text{Survival}(p,r)=\Pr(\mathrm{TTF}>r\mid y_{0}=1)
\]
(i.e., the fraction of initially-correct examples whose first failure occurs *after* round `r`, or never-fail). When reporting a “mean TTF”, we use a **truncated** mean over `1..R` plus a separate NeverFail/Survival@R term, rather than treating `never-fail` as a literal numeric time-to-event.
...
Huang et al. \cite{huang2026vulnerabilityllmsbeliefsystems} systematize persuasion strategies under an SMCR framework and analyze belief changes in multi-turn interventions. GALILEO complements this line by providing a reproducible, ground-truth-centered protocol with explicit dynamics metrics (survival/TTF) and recovery evaluation.
...
- **Dynamics-first measurement (multi-round):** rather than a single rebuttal, we measure how repeated pressure erodes correctness over rounds via **survival curves** and **time-to-first-failure** trajectories.
...
- **Reproducible exports:** we provide standardized per-run exports (survival/TTF/recovery) so claims can be verified directly from artifacts.
...
- Report the same survival/TTF/recovery views for additional tasks (QA/MCQA/OpenQA) and at least one additional model family.
...
**Cross-family visualization (control vs strong persona).** A compact view of Survival@5 for the NRC vs a strong persona (Logical Trap) across families is in `docs/paper/figures/cross_family_survival_r5_control_vs_logicaltrap_seed1-2_20260221.svg`. **Interpretation:** our cross-family claim is about the *within-family gap* (persona − control) being consistently negative under an identical protocol, not about comparing absolute Survival@5 levels across model families. As of `20260221`, this cross-family set includes **Qwen2.5‑14B‑Instruct**, **DeepSeek‑LLM‑7B‑Chat**, **Yi‑6B‑Chat**, and **Phi‑3.5‑mini‑instruct** (all seeds 1–2) in addition to Mistral/Llama/Phi‑3‑mini/Zephyr.

**Caveat (context-length feasibility / capped generations).** Some model families can trigger vLLM batch-wise generation caps when the packed prompt length leaves insufficient headroom under `max_model_len` (e.g., log lines of the form `requested max_tokens=... capped to 1`). When this occurs, the run is **not protocol-comparable** (it is effectively forced into one-token “answers”) and we exclude it from paper-ready Tier‑1 evidence.

Concretely, we observed this failure mode in both (i) Phi‑3.5‑mini and (ii) an attempted StableLM‑2‑1.6B cross-family run on our current stack, so we treat it as a **feasibility/settings issue** rather than evidence about the model’s true multi-turn robustness.

In our Phi‑3.5‑mini runs, the log reports `requested max_tokens=2048 capped to 1 (max_model_len=4096)`, and the runs show near-total collapse (Survival@5≈0). We therefore treat Phi‑3.5‑mini as a stress-test signal rather than a headline quantitative comparison until we confirm that the generation/length settings are fully comparable.

Concretely, Phi-3-mini also shows a sizeable gap under the same protocol (seeds 1–2; see `docs/paper/artifacts/tier1_phi3mini_seed1-2_survival_summary_20260217.csv`). Mistral-Nemo-Instruct-2407 exhibits the same qualitative separation (seeds 1–2; see `docs/paper/artifacts/tier1_mistralnemo_seed1-2_survival_summary_20260217.csv`). Zephyr-7B-beta matches the same pattern (seeds 1–2; see `docs/paper/artifacts/tier1_zephyr7b_seed1-2_survival_summary_20260218.csv`).
...
---|---|---|---
C1 (Dynamics): failures are multi-turn trajectories; survival/TTF needed beyond initial accuracy. | `fig:survival-curves-rounds` → `docs/paper/figures/survival_curves_rounds_seed1-4_20260209.svg`; `fig:tof-delta-fail1` → `docs/paper/figures/tof_personawise_fail1_delta_seed1-4_20260209.svg` | `docs/paper/artifacts/survival_curve_personawise_control_vs_persona_seed1-4_mean_std_20260209.csv`; `docs/paper/artifacts/tof_personawise_fail1_never_control_vs_persona_seed1-4_mean_std_20260209.csv` | Local reproducibility check (requires `results_paper/` present): `python3 scripts/validate_paper_exports.py --results_root results_paper --check_runner_parity` (see `results_paper/GLOBAL_VALIDATE.log`)
...
- **Conditioning / selection effects.** Because our headline multi-turn metrics are reported **conditional on initial correctness** (the subset a model answered correctly at round 0), they characterize *robustness given that the model initially knew the answer*, not end-to-end task performance. This conditioning can induce selection effects (e.g., models with lower initial accuracy may be evaluated on an easier subset), so **conditional robustness is not directly comparable across models with very different round‑0 accuracy unless the conditioning set is held fixed** (Shared‑$C$). We therefore report initial accuracy alongside conditional robustness, emphasize within‑model **persona–control deltas** computed on the same conditioning set, and encourage complementary reporting of **unconditional** multi-turn accuracy for deployment-oriented comparisons.
...
- **Prompting / chat-template dependence.** Pressure susceptibility is not purely a property of model weights: it can vary with the system prompt, chat template, answer-format constraints (e.g., mandatory boxing), and decoding settings. We therefore (i) keep prompts/templates fixed within a paper-reported comparison, (ii) report persona–control deltas under matched prompts/decoding, and (iii) log runner metadata (chat template, decoding, and max-token settings) in `paper_exports/metadata.json` for auditability.
...
- **Token-budget / generation-cap artifacts (inference stack).** For some model stacks, long prompts can trigger runtime feasibility constraints that silently **cap** the generation length (e.g., `max_tokens` reduced to 1 to satisfy context/KV-cache bounds). This can artificially inflate flip rates (a 1-token output is unlikely to include a valid boxed final answer) and therefore is not a meaningful measure of “pressure susceptibility.” To avoid citing such artifact-driven collapses, we (i) record prompt/decoding metadata in `paper_exports/metadata.json`, (ii) validate staged runs under `results_paper/` before using them in figures/tables, and (iii) quarantine incomplete or generation-capped runs as non-citable.
...
- **Responsible release.** When releasing artifacts, we recommend minimizing potentially weaponizable details (e.g., releasing code to *generate* persona prompts rather than a curated “best-of” manipulation script), and limiting shared conversation logs to the minimal excerpts needed for scientific verification, with redaction of any sensitive or personally identifying content.
