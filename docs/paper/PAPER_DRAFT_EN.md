...
Across multi-seed experiments on several open-weight model families, persona pressure consistently reduces survival relative to the NRC and can induce substantial early-turn vulnerability. In our main aggregated setting (seed1–4; **persona-weighted** aggregate computed as an **equal-weight mean over personas** of the corresponding persona–NRC deltas on matched initially-correct subsets, then averaging over seeds), persona pressure reduces **Survival@5** by **22.8 percentage points** and increases **Fail@1** by **6.9 points** relative to the NRC. Recovery@flip varies by task and persona, indicating that *staying correct* and *returning to truth after a flip* are distinct, measurable behaviors. We release an auditable evaluation pipeline with paper-ready exports (CSV artifacts + figure sources) to support reproduction and reviewer verification.
