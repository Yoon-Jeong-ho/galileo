...
GALILEO reports three complementary outcomes (all conditional on initial correctness): **Survival@r** (staying correct through round *r*), **Fail@1 / time-to-first-failure** (early-turn vulnerability), and **Recovery@flip** (returning to the correct answer after a flip on a final neutral recovery prompt). Across multi-seed experiments on several open-weight model families, we find that persona pressure consistently degrades robustness beyond NRC drift; in our main aggregated setting (Qwen2.5‑7B; 4 seeds; persona‑weighted aggregate over tasks/personas), pressure reduces Survival@5 by **22.8 pp** (vs. NRC) and increases Fail@1 by **6.9 pp**, while recovery after a flip is far from guaranteed. We release an auditable evaluation pipeline with paper-ready exports to support reproduction and reviewer verification.
