...
We present **GALILEO**, a benchmark and pipeline for measuring *multi-turn robustness given initial correctness*. GALILEO first conditions evaluation on the **initially-correct** subset, then applies five adversarial personas for up to **five rounds**. We report (i) **survival curves** (fraction remaining correct through round *r*), (ii) **turn-of-failure (TOF)** (the first flip round), and (iii) **recovery@flip**—accuracy on a dedicated recovery prompt **conditional on flipping**. To separate persona-induced effects from generic multi-turn variance, we include a matched **Neutral Re-asking Control** with the same round structure but neutral turns that introduce no new task-relevant evidence. For stable automatic scoring across tasks and multi-turn logs, we standardize final answers to `\boxed{...}` and use **boxed-priority extraction** (when multiple boxes appear, the **last** box is treated as final).

In multi-seed experiments across several open-weight model families, persona pressure reduces robustness relative to the Neutral Re-asking Control, with substantial early-turn vulnerability for some personas. Recovery@flip varies by task and persona, indicating that *staying correct* and *returning to truth after a flip* are distinct behavioral axes.
