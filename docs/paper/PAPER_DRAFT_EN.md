...
GALILEO reports three complementary outcomes (all **conditional on initial correctness**): (i) **survival curves**, where Survival@r is the fraction that stays correct at *every* turn through round r (not merely accuracy at round r); (ii) the **turn-of-failure (TOF)** distribution, including Fail@1 for immediate vulnerability; and (iii) **Recovery@flip**, measured on a final neutral recovery prompt after round 5, conditional on having flipped at least once during rounds 1–5.

Across multi-seed experiments on several open-weight model families, persona pressure consistently reduces survival relative to the NRC and increases early-turn vulnerability. In our main aggregated setting (4 seeds), persona pressure reduces Survival@5 by 22.8 percentage points and increases Fail@1 by 6.9 points relative to the NRC. Recovery@flip varies by task and persona, indicating that *staying correct* and *returning to truth after a flip* are distinct behaviors. We release an auditable evaluation pipeline with paper-ready exports (CSV artifacts + figure sources) to support reproduction and reviewer verification. (Roadmap: protocol + NRC in Fig.~\ref{fig:protocol}; main deltas in Table~\ref{tab:tablew} and Fig.~\ref{fig:tablew-effect-deltas}; survival/TOF/recovery in Figs.~\ref{fig:survival-curves-rounds}, \ref{fig:tof-delta-fail1}, \ref{fig:recovery-delta}.)
