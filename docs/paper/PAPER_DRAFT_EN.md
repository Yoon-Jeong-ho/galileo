...
Across multi-seed Qwen runs (seeds 1–4) and two additional model families (Mistral-7B and Llama-3.1-8B, seeds 1–2), we observe consistent persona-dependent degradation patterns relative to the Neutral Re-asking Control. Recovery conditional on flip varies by task and setting, underscoring that robustness (staying correct) and recovery are distinct axes.
