...
GALILEO reports three complementary outcomes (all conditional on initial correctness): **Survival@r** (staying correct at every turn through round *r*), **Fail@1 / time-to-first-failure** (early-turn vulnerability), and **Recovery@flip** (returning to the correct answer on a final neutral recovery prompt, conditional on having flipped during rounds 1..R). Across multi-seed experiments on several open-weight model families, we find that persona pressure consistently degrades robustness beyond NRC drift; in our main aggregated setting (Qwen2.5‑7B; 4 seeds; persona‑weighted aggregate over tasks/personas), pressure reduces Survival@5 by **22.8 pp** (vs. NRC) and increases Fail@1 by **6.9 pp**, while recovery after a flip is far from guaranteed. **Proof pointers:** protocol in Fig.~\ref{fig:protocol}; control-vs-persona summary in Table~\ref{tab:tablew} (and effect deltas in Fig.~\ref{fig:tablew-effect-deltas}); dynamics in Figs.~\ref{fig:survival-curves-rounds} and \ref{fig:tof-delta-fail1}; recovery in Fig.~\ref{fig:recovery-delta}. We release an auditable evaluation pipeline with paper-ready exports to support reproduction and reviewer verification.
