...
Large language models (LLMs) can abandon previously correct answers when confronted with social or rhetorical pressure (e.g., repeated denial, appeals to authority, or persuasive reframing), even on tasks with clear ground truth. While prior work studies sycophancy and persuasion-related vulnerabilities, evaluation protocols for *ground-truth* tasks rarely characterize (i) **when** failures first occur over interaction rounds, (ii) **how** robustness decays across turns, and (iii) **whether** models can *recover* after being misled.

We present **GALILEO**, a benchmark and pipeline for measuring multi-turn robustness under adversarial *persona-based* pressure. GALILEO (1) filters to initially-correct examples, (2) applies five adversarial personas for up to five rounds, and reports **survival curves** (probability an example remains correct through round *r*) and **turn-of-failure (TOF)** (the first round where a correct answer flips to incorrect), and (3) measures **recovery accuracy conditional on flip** via a dedicated recovery prompt. **All multi-turn metrics are computed on the initially-correct subset**, so the survival/TOF/recovery quantities isolate *robustness given initial correctness* rather than conflating robustness with initial accuracy. To separate persona-driven effects from generic multi-turn variance, we include a **Neutral Re-asking Control** with the same round structure but non-adversarial user turns (no new task-relevant evidence). For stable automatic scoring across tasks and multi-turn logs, we standardize final answers to `\boxed{...}` and use boxed-first extraction.

Across multi-seed runs and multiple model families, persona pressure consistently reduces robustness relative to the Neutral Re-asking Control, with substantial early-turn vulnerability. Recovery conditional on flip varies by task and persona, demonstrating that *staying correct* and *returning to truth* are distinct behavioral axes.
