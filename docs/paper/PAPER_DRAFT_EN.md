...
Concretely, this also motivates our **Neutral Re-asking Control**: we separate pressure-induced failure dynamics from generic multi-turn drift by holding the round structure fixed while removing adversarial evidence/pressure in the user turns (control-vs-persona comparisons: Table~\ref{tab:tablew}, Fig.~\ref{fig:tablew-effect-deltas}).
