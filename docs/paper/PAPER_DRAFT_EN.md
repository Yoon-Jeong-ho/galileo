...
We adopt the same core lens—**failure as an event over turns**—but make two design choices aimed at reviewer-checkability on ground-truth tasks: (i) we keep the primary dynamics metrics **non-parametric and discrete-time** (survival curves / TOF / recovery with explicit right-censoring at the dialogue horizon), rather than depending on a particular parametric hazard model; and (ii) we add a **matched Neutral Re-asking Control** to isolate persona-induced effects from generic multi-turn variance.

Concretely, the control repeats the same round structure under the same decoding and answer-format constraints, but replaces adversarial personas with **neutral re-check requests** that add **no new task-relevant evidence**. This makes it a *matched drift baseline*: it controls for re-answering, answer-format effects, and long-context accumulation while holding the underlying information state fixed. Unlike a “weaker persona” (e.g., simple denial), it removes the social-pressure mechanism entirely; unlike evidence-injection / belief-revision settings, it avoids confounding robustness with rational updating.
