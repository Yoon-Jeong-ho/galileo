...
We adopt the same core lens—**failure as an event over turns**—but make two design choices aimed at reviewer-checkability on ground-truth tasks: (i) we keep the primary dynamics metrics **non-parametric and discrete-time** (survival curves / TOF / recovery with explicit right-censoring at the dialogue horizon), rather than depending on a particular parametric hazard model; and (ii) we add a **Neutral Re-asking Control** with identical round structure to separate persona-induced effects from generic multi-turn drift.

Crucially, this baseline must be *matched* to persona arms in turns, decoding, and answer-format constraints; otherwise, differences can be driven by re-answering or formatting drift alone. At the same time, it must remain non-adversarial (unlike Simple Denial) and evidence-free (unlike belief-revision settings), so it estimates generic multi-turn variance without introducing new pressure or information.
