...
We introduce **GALILEO**, a reproducible protocol for measuring *multi-turn robustness conditional on initial correctness* across ground-truth tasks (math, extractive QA, multiple-choice QA, open-domain QA). We first filter to the **initially-correct** subset, then apply five pressure personas for up to **five rounds**, paired with a matched **Neutral Re-asking Control** that holds dialogue length and decoding fixed while adding no new task-relevant evidence. To isolate persona effects from generic multi-turn drift, **persona–control comparisons are always computed on the same initially-correct subset**. We report three complementary outcomes: (i) **survival curves** (fraction remaining correct through round *r*), (ii) **turn-of-failure (TOF)** / **Fail@1** (the first correct\(\to\)incorrect flip round), and (iii) **recovery@flip**—\(\Pr(\text{correct on recovery prompt}\mid\text{flipped at least once})\). We interpret flips as a discrete **time-to-event** process with **right-censoring** at the dialogue horizon.
...
**Cross-family visualization (control vs strong persona).** A compact view of Survival@5 for the Neutral Re-asking Control vs a strong persona (Logical Trap) across families is in `docs/paper/figures/cross_family_survival_r5_control_vs_logicaltrap_seed1-2_20260219.svg`. **Interpretation:** our cross-family claim is about the *within-family gap* (persona − control) being consistently negative under an identical protocol, not about comparing absolute Survival@5 levels across model families. As of `20260219`, this cross-family set includes **Qwen2.5‑14B‑Instruct** and **DeepSeek‑LLM‑7B‑Chat** (both seeds 1–2) in addition to Mistral/Llama/Phi/Zephyr.
