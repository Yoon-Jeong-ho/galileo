...
In extractive QA, strict exact-match (EM) scoring can label some *near-miss* flips as failures (e.g., over-answers that contain the correct entity plus modifiers). To avoid overstating “belief change,” we qualitatively bucket flip cases using token-overlap (F1) as a diagnostic: **boundary/overanswer** (high F1; e.g., F1≥0.8), **partial-overlap** (moderate F1; e.g., 0.5≤F1<0.8), and **semantic-change** (low F1; e.g., F1<0.5, typically different entities/labels). Separately, we treat rare *format/extraction failure* artifacts (e.g., stray `\\text{...}` strings) as their own cases rather than semantic flips. This taxonomy is complementary to the main survival/TOF metrics: it helps interpret *what kind* of errors the Neutral Re-asking Control produces (often partial-overlap) versus persona pressure (semantic-change dominates). It is intended as an interpretability aid, not a replacement for the quantitative claims. For MCQA, the answer is a discrete label, so label flips correspond to unambiguous semantic changes. In our qualitative samples, ARC-Easy flips under persona pressure frequently manifest as pure label flips, and the same pattern appears across multiple seeds in the Qwen2.5-7B runs. We use such examples as illustrative snapshots; the core claims remain grounded in the quantitative survival/TOF/recovery exports. Rare extraction/format artifacts can also appear in flip snapshots (e.g., a handful of `\\text{...}` strings, observed only in SVAMP in our samples), and we treat these as separate “format/extraction failure” cases when interpreting flips.
