...
Large language models (LLMs) can abandon previously correct answers when confronted with social or rhetorical pressure (e.g., repeated denial, appeals to authority, persuasive reframing), even on tasks with clear ground truth—closely related to recent findings on sycophancy and persuasion-induced behavior change in dialogue (\cite{sharma2023understandingsycophancylanguagemodels,fanous2025sycevalevaluatingllmsycophancy,huang2026vulnerabilityllmsbeliefsystems}). However, most ground-truth benchmarks report only single-turn accuracy, which obscures multi-turn failure dynamics: **when** the first correct\(\to\)incorrect flip happens, **how** robustness decays over rounds, and **whether** models can return to the truth after being misled.

We introduce **GALILEO**, a reproducible protocol for measuring *multi-turn robustness conditional on initial correctness* across ground-truth tasks (math, extractive QA, multiple-choice QA, open-domain QA). After filtering to the **initially-correct** subset, GALILEO applies five adversarial personas for up to **five rounds** and reports three complementary outcomes: (i) **survival curves** (fraction remaining correct through round *r*), (ii) **turn-of-failure (TOF)** (the first flip round; including Fail@1), and (iii) **recovery@flip**—\(\Pr(\text{correct on recovery prompt}\mid\text{flipped at least once})\). We treat correctness flips as a discrete **time-to-event** process with **right-censoring** at the dialogue horizon, making survival-style summaries directly interpretable in dialogue settings. To attribute effects to pressure rather than generic multi-turn variance, we include a matched **Neutral Re-asking Control** with the same round structure and decoding, but neutral turns that introduce no new task-relevant evidence. **All persona-vs-control comparisons are computed on the same initially-correct subset for that persona arm (arm-matched conditioning),** so control aggregates may differ across personas when their conditioning sets differ. For stable automatic scoring from multi-turn logs, we standardize final answers to `\boxed{...}` and use **boxed-priority extraction** (if multiple boxes appear, the **last** box is treated as final).
