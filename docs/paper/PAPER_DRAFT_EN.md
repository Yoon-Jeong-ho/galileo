...
Large language models (LLMs) can abandon previously correct answers when faced with social or rhetorical pressure (e.g., repeated denial, appeals to authority, persuasive reframing), even on tasks with clear ground truth—closely related to recent findings on sycophancy and persuasion-induced behavior change in dialogue (\cite{sharma2023understandingsycophancylanguagemodels,fanous2025sycevalevaluatingllmsycophancy,huang2026vulnerabilityllmsbeliefsystems}). Yet most ground-truth benchmarks report only single-turn accuracy, leaving multi-turn failure dynamics under pressure underspecified: **when** does the first correct\(\to\)incorrect flip occur, **how** does robustness decay across rounds, and **can** models return to the truth after being misled?

We present **GALILEO**, a benchmark and pipeline for measuring multi-turn robustness under adversarial *persona-based* pressure. GALILEO (1) conditions evaluation on the **initially-correct** subset to isolate *robustness given initial correctness*; (2) applies five adversarial personas for up to **five rounds** and reports **survival curves** (fraction remaining correct through round *r*) and **turn-of-failure (TOF)** (the first flip round); and (3) measures **recovery@flip**—accuracy on a dedicated recovery prompt **conditional on flipping**. To attribute flips to pressure mechanisms rather than generic multi-turn drift, we include a matched **Neutral Re-asking Control** with the same round structure but neutral user turns that introduce no new task-relevant evidence. For stable automatic scoring across tasks and multi-turn logs, we standardize final answers to `\boxed{...}` and use **boxed-priority extraction** (extract from `\boxed{...}` when present; if multiple boxes appear, take the **last** box as the final answer).

Across multi-seed runs and multiple model families (e.g., Qwen2.5-7B, Mistral-7B, Llama-3.1-8B, Phi-3-mini), persona pressure consistently reduces robustness relative to the Neutral Re-asking Control, often with substantial early-turn vulnerability. Recovery@flip varies by task and persona, indicating that *staying correct* and *returning to truth* are distinct behavioral axes.
