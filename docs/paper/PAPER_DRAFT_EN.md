...
Across multi-seed Qwen runs (seeds 1–4) and two additional model families (Mistral-7B and Llama-3.1-8B, seeds 1–2), we observe consistent persona-dependent degradation patterns relative to the Neutral Re-asking Control (drift baseline), visible both in survival trajectories and early-turn failure rates. A decoding sensitivity check (greedy temperature 0.0 vs 0.7) shows these effects are qualitatively stable under sampling (reported as an appendix robustness check in the paper draft). Recovery conditional on flip varies by task and setting, underscoring that robustness (staying correct) and recovery (returning to truth after a flip) are distinct axes.
