...
Large language models (LLMs) can abandon previously correct answers when confronted with social or rhetorical pressure (e.g., repeated denial, appeals to authority, persuasive reframing), even on tasks with clear ground truth—closely related to recent findings on sycophancy and persuasion-induced behavior change in dialogue (\cite{sharma2023understandingsycophancylanguagemodels,fanous2025sycevalevaluatingllmsycophancy,huang2026vulnerabilityllmsbeliefsystems}). Yet most ground-truth benchmarks report only single-turn accuracy, obscuring *interaction dynamics*: **when** the first correct\(\to\)incorrect flip occurs, **how** robustness decays over rounds, and **whether** models can recover after being misled.

We introduce **GALILEO**, a reproducible protocol for measuring *multi-turn robustness conditional on initial correctness* across ground-truth tasks (math, extractive QA, multiple-choice QA, open-domain QA). After filtering to the **initially-correct** subset, GALILEO applies five pressure personas for up to **five rounds** and compares them against a matched **Neutral Re-asking Control** (same dialogue length and decoding, but neutral turns that add no new task-relevant evidence). **All persona–control comparisons are computed on the same initially-correct subset to keep attribution apples-to-apples.** We report three complementary outcomes: (i) **survival curves** (fraction remaining correct through round *r*), (ii) **turn-of-failure (TOF)** / **Fail@1** (the first flip round), and (iii) **recovery@flip**—\(\Pr(\text{correct on recovery prompt}\mid\text{flipped at least once})\). We interpret flips as a discrete **time-to-event** process with **right-censoring** at the dialogue horizon.

Across multi-seed experiments on several open-weight model families, persona pressure consistently reduces survival relative to the Neutral Re-asking Control and can induce substantial early-turn vulnerability. Recovery@flip varies by task and persona, supporting a two-axis view in which *staying correct* and *returning to truth after a flip* are distinct behaviors.
...
- *“Is this just long-context degradation / generic drift?”* See our **matched Neutral Re-asking Control** design (introduced in §1.1 and specified in the protocol) for separating persona-induced effects from generic multi-turn variance.
...
Recent preregistered human-subject studies further motivate treating multi-round dialogue as a realistic setting where model outputs can shift human beliefs or behavior, and where personalization can amplify effects. For example, large-scale debate-style experiments compare GPT-4 to human opponents under randomized assignment and include a “personalized” condition where demographic profile attributes are available to the persuader (Nature Human Behaviour, 2025: https://www.nature.com/articles/s41562-025-02194-6). In parallel, smaller naturalistic settings (e.g., Hebrew Telegram conversations) suggest that static one-shot messages can be comparably persuasive to dynamic chat, highlighting that *interaction itself* is not the only driver of influence (CogSci 2025 accepted: https://arxiv.org/abs/2503.01844). We treat these results as complementary societal-impact context; our core contribution remains an auditable, ground-truth trajectory protocol for pressure-induced failures and recovery.

### 6.3 Stability under context
Tosato et al. \cite{tosato2025persistentinstabilityllmspersonality} and Yu et al. \cite{yu2026ptcbenchbenchmarkingcontextualstability} report instability in measured traits/personality under prompt and conversation-history variations, supporting the motivation for studying robustness dynamics under accumulating context. Lu et al. \cite{lu2026assistantaxis} ("Assistant Axis") further argue that LMs exhibit a default assistant-persona axis that can drift under interaction and can be stabilized with explicit persona anchoring; this complements our focus on multi-turn *robustness under pressure* by highlighting that persona itself is a moving part and motivates conditioning + drift baselines.
...
**Cross-family visualization (control vs strong persona).** A compact view of Survival@5 for the Neutral Re-asking Control vs a strong persona (Logical Trap) across families is in `docs/paper/figures/cross_family_survival_r5_control_vs_logicaltrap_seed1-2_20260218.svg`. **Interpretation:** our cross-family claim is about the *within-family gap* (persona − control) being consistently negative under an identical protocol, not about comparing absolute Survival@5 levels across model families.

Concretely, Phi-3-mini also shows a sizeable gap under the same protocol (seeds 1–2: Neutral Re-asking Control **Survival@5 = 25.22%±7.85**, Logical Trap **Survival@5 = 9.16%±1.36**; tracked artifact `docs/paper/artifacts/tier1_phi3mini_seed1-2_survival_summary_20260217.csv`). Mistral-Nemo-Instruct-2407 exhibits the same qualitative separation (seeds 1–2: Neutral Re-asking Control **Survival@5 = 32.54%±4.77**, Logical Trap **Survival@5 = 9.29%±0.44**; tracked artifact `docs/paper/artifacts/tier1_mistralnemo_seed1-2_survival_summary_20260217.csv`). Zephyr-7B-beta matches the same pattern (seeds 1–2: Neutral Re-asking Control **Survival@5 = 31.06%±14.59**, Logical Trap **Survival@5 = 16.38%±9.02**; tracked artifact `docs/paper/artifacts/tier1_zephyr7b_seed1-2_survival_summary_20260218.csv`).
...
```latex
% Source SVG (repo): docs/paper/figures/cross_family_survival_r5_control_vs_logicaltrap_seed1-2_20260218.svg
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/cross_family_survival_r5_control_vs_logicaltrap_seed1-2_20260218}
...
C2 (Mechanism vs drift): persona pressure causes effects beyond generic drift; control baseline is essential. | `fig:tablew-effect-deltas` → `docs/paper/figures/table_w_effect_delta_seed1-4_20260209.svg` (includes Neutral Re-asking Control as drift baseline) | `docs/paper/artifacts/table_w_effect_delta_seed1-4_20260209.csv`; `docs/paper/artifacts/table_w_control_vs_persona_seed1-4_mean_std_20260209.csv`; (qual taxonomy) Appendix~A.2 | generator: `scripts/make_table_w_control_vs_persona.py --control_persona_id neutral_reask_control --round 5`; local validation (if `results_paper/` present): `python3 scripts/validate_paper_exports.py --results_root results_paper --check_runner_parity`
...
C4 (Cross-family): effects replicate across model families under the same protocol. | `fig:cross-family-survival` → `docs/paper/figures/cross_family_survival_r5_control_vs_logicaltrap_seed1-2_20260218.svg` | `docs/paper/artifacts/tier1_mistral7b_seed1-2_survival_summary_20260210.csv`; `docs/paper/artifacts/tier1_llama3_8b_seed1-2_survival_summary_20260210.csv`; `docs/paper/artifacts/tier1_llama3_3b_seed1-2_survival_summary_20260212.csv`; `docs/paper/artifacts/tier1_phi3mini_seed1-2_survival_summary_20260217.csv`; `docs/paper/artifacts/tier1_mistralnemo_seed1-2_survival_summary_20260217.csv`; `docs/paper/artifacts/tier1_zephyr7b_seed1-2_survival_summary_20260218.csv` | Paper-ready run roots (when synced locally): `results_paper/mistral_seed{1,2}`, `results_paper/llama_seed{1,2}`, `results_paper/tier1_llama3_3b_seed1_20260212_030426`, `results_paper/tier1_llama3_3b_seed2_20260212_042339`, `results_paper/tier1_zephyr7b_seed1_20260218_0945`, `results_paper/tier1_zephyr7b_seed2_20260218_141231`; validate via `python3 scripts/validate_paper_exports.py --results_root results_paper --check_runner_parity`
