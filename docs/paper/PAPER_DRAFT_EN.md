...
**Caveat (Phi‑3.5‑mini).** In our Phi‑3.5‑mini runs, the log reports `requested max_tokens=2048 capped to 1 (max_model_len=4096)`, and the runs show near-total collapse (Survival@5≈0). We therefore treat Phi‑3.5‑mini as a stress-test signal rather than a headline quantitative comparison until we confirm that the generation/length settings are fully comparable.
