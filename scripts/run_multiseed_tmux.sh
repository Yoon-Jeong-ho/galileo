...
# Multi-seed runner for paper-ready statistics.
# Runs 7B and 14B (no 32B) across multiple seeds on a fixed GPU set.
...
# Strict unified data dir (no legacy/pilot *_val_50 files)
...
to_array() {
  local s="$1"; IFS=, read -r -a arr <<< "$s"; echo "${arr[@]}";
}
...
run_one() {
  ...
  # paper exports (curves + turn-of-failure + flip sheet)
...
for s in "${seeds[@]}"; do
  run_one "$s" "$MODEL_7B"  "7b"
  run_one "$s" "$MODEL_14B" "14b"
done
